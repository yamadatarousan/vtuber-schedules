  ...
  try {
    ...
    for (const channelId of channelIds) {
      const response = await fetch(
        `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelId}&eventType=live&type=video&maxResults=5&key=${apiKey}`
      );
      if (!response.ok) {
      ...
      console.log(`YouTube search response (live, ${channelId}):`, data);
    ...
    if (!videoIds.length) {
      const schedules = await query('SELECT s.*, v.name, v.vtuber_group FROM schedules s JOIN vtubers v ON s.vtuber_id = v.id WHERE type = ?', ['live']);
      console.log('DB schedules (live):', schedules);
    ...
    console.log('YouTube videos response (live):', videosData);

    for (const item of videosData.items) {
      if (!item.snippet || !item.id) continue;
      const startTime = item.liveStreamingDetails?.actualStartTime || item.snippet.publishedAt;
    ...
    const schedules = await query('SELECT s.*, v.name, v.vtuber_group FROM schedules s JOIN vtubers v ON s.vtuber_id = v.id WHERE type = ?', ['live']);
    console.log('Final schedules (live):', schedules);
    return NextResponse.json(schedules || []);
  } catch (error: any) {
    console.error('Error in /api/live:', error.message);
    return NextResponse.json([], { status: 200 });
  }
}